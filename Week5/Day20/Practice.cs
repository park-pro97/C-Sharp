...
테이블에 데이터 추가하기 (INSERT문)

UPDATE문   -WHERE절로 일부분만 수정
...
시퀀스란 - 번호 자동 생성기.--객체임   (교재 348P)

도구를 사용해서 바로 만들 수 있지만 코드로도 작성이 가능하다.


프로그램의 꽃 - 제약.
제약 조건이란?  테이블 열에 저장될 데이터의 특성, 조건을 지정

------------------------------------------------------------------------------------------------------------
                                   [제약 조건의 종류]  -지약: CONSTRAINT

NOT NULL    - 지정한 열에 NULL을 허용하지 않음 / NULL을 제외한 데이터의 중복은 허용
UNIQUE       - 지정한 열이 유일한 값을 가져야 한다. (중복X) 단, NULL은 중복에서 제외
PRIMARY KEY - 지정한 열이 유일한 값이면서 NULL을 허용하지 않음. (테이블당 하나만 지정 가능)
FOREIGN KEY - 다른 테이블의 열을 참조하여 존재하는 값만 입력할 수 있다.
CHECK         - 설정한 조건식을 만족하는 데이터만 입력 가능


중요중요  FOREIGN KEY[포린 키]  중요중요
기본값을 정하는 DEFAULT   -  저장값이 없을 경우 기본값을 사용


제약에 이름을 지정할 수 있다.!!!!!!

포린키에 대해 집중적으로 한다고 하셨음.

점심 먹고는 제약에 관한 실습문제 혼자 좀 해보라고 하셨음
